...
3. 조사관들은 병렬 조회 후 결과 파일 경로를 리더/tax-advisor에게 통지.
...
4. `legal-analyst`가 법률+세무를 `_workspace/03_analyst_report.md`에 종합 → 사용자에게 요약.
5. 팀 정리(`TeamDelete`).
...
- 판례 미발견: 검색범위 확대(`search=2`) 후에도 없으면 "관련 판례 확인 안 됨"으로 명시.
